...
using SharpDX;
using Geometry2 = HoolaLucian.Geometry2;
...
namespace HoolaLucian
{
    public class Program
    {
        ...
        private static int QMinDist => Menu.Item("QMinDist").GetValue<Slider>().Value;
        ...
        private static void OnMenuLoad()
        {
            ...
            Combo.AddItem(new MenuItem("QMinDist", "Min dist from enemies", true).SetValue(new Slider(375, 325, 525)));
        ...
        private static Vector3 GetSafeTumblePos(Obj_AI_Hero target)
        {
            ...
            var myTumbleRangeCircle =
                new Geometry2.Circle(ObjectManager.Player.ServerPosition.To2D(), 350f).ToPolygon().ToClipperPath();
        ...
        private static void OnDoCastDelayed(GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (args.Target is Obj_AI_Hero)
            {
                ...
                if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo && target.IsValid)
                {
                    ...
                    {
                        E.Cast(GetSafeTumblePos((Obj_AI_Hero)target));
                    }
        ...
        static void SemiUlt()
        {
            ...
            if (target.IsValid && R.IsReady()
                && !ObjectManager.Player.HasBuff("LucianR") && !target.IsDead && !target.IsZombie && !W.IsReady())
            {
